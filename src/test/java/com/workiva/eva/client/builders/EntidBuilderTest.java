...
public class EntidBuilderTest {
  ...
  public void executeTest() {
    Keyword ident = Keyword.intern("some", "keyword");
    EntidBuilder invokeBuilder =
        new EntidBuilder(db, ident).withRequestHeaders(HEADERS).withCorrelationId("cid");

    Map results = (Map) invokeBuilder.execute();
    Assert.assertEquals(results.get("db"), db);
    Assert.assertEquals(results.get("ident"), ident);
  ...
  public void executeStrictTest() {
    ...
    IdentBuilder invokeBuilder =
        new IdentBuilder(db, ident).asStrict().withRequestHeaders(HEADERS).withCorrelationId("cid");

    Map results = (Map) invokeBuilder.execute();
    Assert.assertEquals(results.get("db"), db);
    Assert.assertEquals(results.get("entid"), ident);
